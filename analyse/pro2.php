...
try 
 ...
 $req = $bdd->query("SELECT distinct(TABLE_NAME) from INFORMATION_SCHEMA.STATISTICS where TABLE_SCHEMA ='users' order by `TABLE_NAME` ASC");
...
while ($resul = $reqdeux->fetch()) {
	...
	$nomcol=$resul["col"];
		...
		if($compte=='date') echo "<tr><td>"." Contient une date "."</td></tr>";
		if($compte=='datetime') echo "<tr><td>"." Contient une date "."</td></tr>";
        ...
        if ($compte=='datetime' or $compte=='date' or $compte=='varchar' ){
            ...
            $condi=$bdd->prepare("SELECT max(length(`$nomcol`)) as lengthmax FROM `$x` ");
            $condi->execute(); 
            ...
            if ($compte=='date' or $compte=='datetime'){
            ...
            $adate = $date->fetchAll();
            $alldate = $adate[0]['lengthmin'];
            echo "<tr><td>"."Date maximum : ".$alldate."</td></tr>";
