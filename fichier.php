<?php
 ...
 //SELECT `chercheur`.`nom`, `chercheur`.`prenom`, `chercheur`.`mail`, `projet`.`nom`, `projet`.`description` FROM chercheur, u_chercheur, projet, u_projet, p_chercheur where `u_chercheur`.`u_id` = 'ULB001' AND `u_chercheur`.`c_id`=`chercheur`.`id` AND `u_projet`.`up_id`='ULB001' AND `u_projet`.`p_id`=`projet`.`p_id` 
